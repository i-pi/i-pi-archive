   ...
   def get_sckintd(self, atom=""):
      ...
      for i in range(self.beads.natoms):
         ...
         for b in range(self.beads.nbeads):
            ...
            if k%2 == 0:
               kcv -= 2*(self.forces.alpha/self.forces.omegan2/9.0)*(f[b,k]*f[b,k]/self.forces.beads.m3[b,k] + f[b,k+1]*f[b,k+1]/self.forces.beads.m3[b,k+1] + f[b,k+2]*f[b,k+2]/self.forcesf.beads.m3[b,k+2])
            else:
