...
class Barostat: 
   def __init__(self, pext = 0.0, dt = 1.0, thermostat=None):
      dset(self,"pext",depend_value(name='pext', value=pext) )
      dset(self,"dt",depend_value(name='dt', value=pext) )     
      self.thermo=termostat
   ...
   def bind(self, atoms, cell, force):
      ...
      self.cell=cell
      self.force=force
      if not self.thermo is None: self.thermo.bind(cell=self.cell)
      pass
