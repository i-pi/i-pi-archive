...
#~
#~
#~ class FFReduced(FFSocket):
   #~ """Interface between the PIMD code and the socket for a single replica of
   #~ the contracted ring polymer.
#~
   #~ Attributes:
      #~ parameters: A dictionary of the parameters used by the driver. Of the
         #~ form {'name': value}.
      ...
      #~ request: During the force calculation step this holds a dictionary
         ...
         #~ Of the form {'atoms': atoms, 'cell': cell, 'pars': parameters,
                      #~ 'status': status, 'result': result, 'id': bead id,
                      #~ 'start': starting time}.
      #~ nreduced: The number of beads in the contracted ring polymer.
   #~ """
#~
   #~ def __init__(self, pars=None, interface=None, nreduced=None):
      #~ """Initialises FFSocket.
#~
      #~ Args:
         #~ pars: Optional dictionary, giving the parameters needed by the driver.
         #~ interface: Optional Interface object, which contains the socket.
         #~ nreduced: Optional integer, giving the number of beads in the
            #~ contracted ring polymer.
      #~ """
#~
      #~ # a socket to the communication library is created or linked
      #~ super(FFReduced,self).__init__(pars, interface)
      #~ self.nreduced = nreduced
      #~ # if nreduced == 0, it will be converted to the full nbeads when it is
      #~ # bound to the multiforce object.
#~
   #~ def copy(self):
      #~ """Creates a deep copy without the bound objects.
#~
      #~ Used in ForceBeads to create a FFReduced for each replica of the system.
#~
      #~ Returns:
         #~ A FFReduced object without atoms or cell attributes.
      #~ """
#~
      #~ # does not copy the bound objects (i.e., the returned forcefield must be bound before use)
      #~ return type(self)(self.pars, self.socket, self.nreduced)
