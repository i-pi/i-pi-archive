...
class Cell(object):
   ...
   def __init__(self, h = numpy.identity(3, float)):      
      ...
      self.h = depend_array(name = 'h', value = h)
      self.p = depend_array(name = 'p', value = numpy.zeros((3,3),float))
      self.V = depend_value(name = 'V', deps=depend_func(func=self.get_volume, dependencies=[depget(self,"h")]))
      self.m = depend_value(name = "m", value = 1.0)
      
      self.kin = depend_value(name = "kin", deps=depend_func(func=self.get_kin, dependencies=[depget(self,"p"),depget(self,"m")]))
      self.ih = depend_value(name = "ih", deps=depend_func(func=self.get_ih, dependencies=[depget(self,"h")]))
