...
class Baro_object:
   def __init__(self):
      self.kind = ""
      self.parameters = {}
      self.pext = np.zeros((3,3))
      self.ref_cell = np.zeros((3,3))
...
class Init_read(xml.sax.handler.ContentHandler):
   ...
   def startElement(self, name, attributes):
      if name == "ensemble_kind":
         ...
         print "Unrecognized opening tag: ", name
         exit()
   ...
   def characters(self, data):
      ...
      elif self.in_fix_com:
         self.fix_com = read_bool(data)
   ...
   def endElement(self, name):
      if name == "ensemble_kind":
         ...
         self.barostat.ref_cell = np.asarray(self.ref_cell)
         self.in_ref_cell = False
      elif name == "Pext": 
         self.barostat.pext = np.asarray(self.Pext)
         self.in_Pext = False
      elif name == "x":
         self.in_x = False
      elif name == "temp":
         ...
         print "Unrecognized closing tag: ", name
         exit()
