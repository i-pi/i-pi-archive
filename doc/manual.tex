...
how strong the thermostat is, which we will set to 25 femtosecond. 
...
simulation time of 5 picoseconds, we therefore need 5000 time steps, i.e.:
...
the data we wish to obtain from it. There are a wide
variety of properties of interest that \ipi can calculate 
and a large number of different output
options, so let us go through them one at a time.
...
First, consider the checkpoint files. These
give a snapshot of the state of the simulation,
...
Trajectory files are used to print out properties
...
Finally, let us consider the properties files. These are usually used to print out
...
Now that we know what each input file works, let us take an
example of an output section and show how the xml input works.
...
\ref{runningsimulations} (if we assume that we are in the {}``tutorial-1''
...
Once the simulation is finished (which should only take a few minutes)
...
has equilibrated, and give us some indication of what the 
...
%Mano's note: Fix help strings of isotope estimators.
