...
using the Silvera-Goldman potential \cite{???}. 
...
suitable for the task at hand, namely doing an equilibration run.
...
fully converged by the end of it. While we could try playing around with the
time step parameter and the thermostat, there are a couple of much simpler ways to
help improve the convergence.

Firstly, we can simply run the simulation for longer. By default the number of
...
distribution. This can be done in the {}``initialize'' tag using the
{}``velocities'' tag:
...
One thing that we will need to do if we are going to run multiple similar
simulations will be to initialize the random number generator with different
seeds, so that they are independent. This is simply done using the
{}``prng'' tag, as follows:
...
unit conversion routines. These have been interfaced with the xml reading code
in such a way that the code automatically knows what dimensionality each
input parameter is and so can automically convert between atomic units 
and a user specified unit. For example, the time step we used in tutorial 0
was 40 atomic time units, but we might instead want to express this in S.I.
units for comparison with another paper. This happens to be approximately
...
and (x2, y2, z2) give the position vector of atoms 1 and 2. Note that this is
...
\textbf{{}``high''}, which amongst other things 
tells \ipi to output statistical data every time step and the
status of the socket communication.

\textbf{PUT SOME MORE INFORMATION HERE!!!!!!!!!!!!!!!!!!!!!!}

All output written to file is dealt with by the {}``output'' tag.
...
To get the same output as previously, we would use the following
syntax:
...
There are several options we can use to customize what data is
output. Firstly, the {}``prefix'' attribute should be set to
...
set by the user. In this case this is done by specifying an
...
which was more expensive to calculate and so we don't want it to
be output every time step like the basic properties given above.
With \ipi this is easy to do, all that is required is to add another
{}``properties'' tag with a different filename, and a larger
{}``stride'' attribute.
...
To print out the forces of one atom, we will use the {}``atom\_f''
...
This also accepts positional arguments, and so this could also be
written as:
...
re-run the simulation. Once the simulation has been run by 
repeating the steps in \ref{run1}, the output of the 
{}``tut1.md'' file should show that all the properties of interest
have converged by the end of the simulation. We are now ready
to start the NPT run.
...
tag field altogether. Similarly, we can adjust the total number of
steps for the actual run, in this case by setting the 
{}``total\_steps'' tag to \textbf{20000}.
...
ensemble is sampled. The first thing that must be done is that 
the {}``mode'' tag must be changed from {}``nvt'' to {}``npt'',
and add a {}``pressure'' tag:
...
In this case, we will use \textbf{{}``bzp''}. 
This gives:
...
we will take its time constant to be \textbf{100 atomic time units}:
...
Finally, we will take the barostat time scale to be \textbf{1e5 atomic time
units}, giving:
...
Another method that can be used is to use the RESTART file as a 
configuration file within the {}``initialize'' tag.
...
for when we modified the RESTART file directly. Next, we replace
the {}``initialize'' tag section with:
...
of the system are actually required to provide converged results.
In the directory {}``tutorial 3'' then, there is simply a stub of
a NVT input file, which can be used to do a fully converged
NPT simulation.

To check that the code is
working correctly, at this state point \cite{That Tuckermann paper}
the volume should be 31 \(\textrm{cm}^3\)/mol and the total energy
should be -48 K.
