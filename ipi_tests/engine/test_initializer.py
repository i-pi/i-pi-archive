...
test_init_file_prms = [
    # (1, 1, 'asdasd', default_cell_mat),
    # (2, 1, 'asdasd', default_cell_mat),
    (1, 2, 'asdasd', default_cell_mat),
]
...
def create_xyz_sample_file(request):
    ...
    natoms, frames, comment, expected_cell = request.param

    filedesc, xyz, atoms_names = xyz_gen.xyz_traj_filedesc(natoms, frames, comment)

    # init_file needs to read from a real file...
    tmp_file = tmp.NamedTemporaryFile(mode='wr', prefix='ipi-testing-tmp', delete=False)
    tmp_file.seek(0)
    ...
    for _fr in xrange(frames):
        ...
        ratoms[-1].q = xyz[_fr*natoms*3:3*(_fr + 1)*natoms]
