...
def totalEnergy(prefix, temp, ss=0, unit=''):
  ...
  try:
    ...
    print('WARNING: No compiled fortran module for fast calculations have been found.\n'
          'Proceeding the calculations using a slow pure python script.')
  ...
  # Some constants
  ePA_p1 = 0.5*nbeads*(Constants.kb*temperature)**2/Constants.hbar**2
  ePA_2 = 1.5*nbeads*Constants.kb*temperature
  eVir_1 = 1.5*Constants.kb*temperature
  ePA_f1 = Constants.kb**2/Constants.hbar**2
  dE_1 = Constants.hbar**2/(24.0*(nbeads*Constants.kb*temperature)**3)
  ...
  while True:  # Reading input files and calculating PPI correction
    ...
    try:
      for i in range(nbeads):
        ...
        #ret = read_file("pdb", ipos[i], output='arrays')
        if natoms == 0:
          #m, natoms = ret["atoms"].m, ret["atoms"].natoms
          m, natoms = ret["masses"], ret["natoms"]
          q = np.zeros((nbeads, 3*natoms))
          f = np.zeros((nbeads, 3*natoms))
        #q[i, :] = ret["atoms"].q
        #f[i, :] = read_file("xyz", ifor[i])["atoms"].q
        q[i, :] = ret["data"]
        f[i, :] = read_file("xyz", ifor[i], output='arrays')["data"]
        #f[i, :] = read_file("pdb", ifor[i], output='arrays')["data"]
    ...
    if ifr >= skipSteps:  # PPI correction
      ...
      if not fast_code:
        ...
        ePA *= ePA_p1 # 0.5*nbeads*(Constants.kb*temperature)**2/Constants.hbar**2
        ePA += ePA_2*natoms + U #0.5*nbeads*(3*natoms)*Constants.kb*temperature + U
        f2ePA = f2*ePA
        eVir /= 2.0*nbeads
        eVir += eVir_1*natoms + U # 0.5*(3*natoms)*Constants.kb*temperature + U
        ...
        ePA *= ePA_f1 # Constants.kb**2/Constants.hbar**2
        ePA += ePA_2*natoms + U #0.5*nbeads*(3*natoms)*Constants.kb*temperature + U
        f2ePA = f2*ePA
        eVir += eVir_1*natoms + U #0.5*(3*natoms)*Constants.kb*temperature + U
      ...
      dE *= dE_1 # Constants.hbar**2/(24.0*(nbeads*Constants.kb*temperature)**3)

      dE = unit_to_user("energy", unit, dE)  # Output in the same unit as potential energy
      eVir = unit_to_user("energy", unit, eVir_av/float(ifr-skipSteps))  # Output in the same unit
      # as potential energy
