...
for each time frame. The output is saved to 'prefix.rdf' file which is created in the
folder which contains the input files. The results are printed out in the format: "time
frame", "conventional RDF", and "ppi RDF".
...
Syntax:
   python energy_ppi.py "prefix" "element A" "element B" "simulation temperature (in Kelvin)" "number of bins for RDF"
...
def RDF(prefix, A, B, temp, nbins, r_min, r_max, ss=0, unit='angstrom'):
