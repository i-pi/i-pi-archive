...
for i in range(cell_no):
   for j in range(cell_no):
      for k in range(cell_no):
         ...
         count += 1
         syst.atoms[count].q.get_array()[:] = numpy.array([i*cell_len, (j+0.5)*cell_len, (k+0.5)*cell_len])
         count += 1
         syst.atoms[count].q.get_array()[:] = numpy.array([(i+0.5)*cell_len, (j+0.5)*cell_len, (k)*cell_len])
         count += 1
         syst.atoms[count].q.get_array()[:] = numpy.array([(i+0.5)*cell_len, (j)*cell_len, (k+0.5)*cell_len])
         count += 1
